function Machinegun(tier, position, clipSize, reloadTime, bulletDurability, knockback, strength, upgradeCost = 100) {
    ...
    this.machinegunImage = loadImage("/images/machinegun.png");
    ...
    this.show = function () {
        ...
        rotate(this.angle);
        image(this.machinegunImage, -this.width / 2, -this.height / 2, this.width, this.height);
        pop();
    ...
    this.onMouseHold = function () {
        if(!this.reloading && gameEngine.tickCount % this.fireRate == 0){
            this.fire();
        }
    ...
    this.upgrade = function () {
        this.tier += 1;
        this.clipSize = Math.floor(this.clipSize * 1.2);
        this.strength *= 1.1;
        this.reloadTime *= .90;
        this.bulletDurability = Math.floor(this.bulletDurability * 1.5);
        //this.knockback *= 1.04;
        this.upgradeCost = Math.floor(this.upgradeCost * 2.1) + (this.tier * 100);
        this.bulletsInClip = this.clipSize;
        this.reloading = false;
        this.kickBack*=.85;
        this.fireRate = max(1, Math.floor(this.fireRate * .95));
        if(this.tier % 3 == 0){
            this.burstFire+=1;
        }
    };
